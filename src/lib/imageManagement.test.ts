...
describe('Image Management Service', () => {
  ...
  describe('generateUniqueImageKey', () => {
    it('should generate unique keys with timestamp', async () => {
      ...
      // Keys should be different due to timestamp, but if generated in same ms, they might be equal
      // So we just verify the format is correct
      expect(key2).toContain('products/prod-001-');
      expect(key2).toContain('.jpg');
    });
